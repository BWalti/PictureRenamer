namespace PictureRenamer.Pipelines
{
    ...
    {
        public static readonly DataflowLinkOptions DataflowLinkOptions =
            new DataflowLinkOptions { PropagateCompletion = true };

        public static readonly ExecutionDataflowBlockOptions SingleExecution =
                ...
                context =>
                    {
                        ...
                        var allFiles = contextDirectory.EnumerateFiles("*", SearchOption.AllDirectories)
                ...
                context =>
                    {
                        if (context.HasError)
                        {
                            ...
                            if ((inputFile.Extension == ".db")
                                || (inputFile.Extension == ".modd")
        ...
        public static IPropagatorBlock<PhotoContext, PhotoContext> CreateMoverAction(
            string alternativeTargetPath = null)
        {
            var output = new BufferBlock<PhotoContext>();
                ...
                context =>
                    {
                        ...
                        var targetFullPath = Path.Combine(
                            targetPath,
                            context.PossibleTargetFileName ?? context.Source.Name);
                ...
                photoContext =>
                    {
                        ...
                        else
                        {
                            ...
                            var dateTime = GetDateTime(photoContext)
                                .FirstOrDefault(s => !string.IsNullOrEmpty(s) && s != "0000:00:00 00:00:00");
        ...
        public static IPropagatorBlock<IEnumerable<PhotoContext>, Dictionary<ulong?, List<PhotoContext>>> FindExactMatches()
        {
            ...
            var input = new ActionBlock<IEnumerable<PhotoContext>>(
                context =>
                    {
                        var grouped =
                            from entry in context
                            group entry by entry.Hash into g
        ...
        {
            yield return photoContext.RgbaImage?.MetaData?.ExifProfile?.GetValue(ExifTag.DateTimeOriginal)
                ?.Value?.ToString();
            yield return photoContext.RgbaImage?.MetaData?.ExifProfile?.GetValue(ExifTag.DateTimeDigitized)
                ?.Value?.ToString();
            yield return photoContext.RgbaImage?.MetaData?.ExifProfile?.GetValue(ExifTag.DateTime)?.Value?.ToString();

            var dateStamp = photoContext.RgbaImage?.MetaData?.ExifProfile?.GetValue(ExifTag.GPSDateStamp)
                ?.Value?.ToString();
            var timeStamp = photoContext.RgbaImage?.MetaData?.ExifProfile?.GetValue(ExifTag.GPSTimestamp)
                ?.Value?.ToString();
        ...
        {
            if (MediaFileNamePattern.IsMatch(info.Extension))
            {
                var detectFileType = FileTypeDetector.DetectFileType(info.OpenRead());
                return AllowedFileTypes.Contains(detectFileType);
            }
        ...
        {
            var transformed = new TransformBlock<PhotoContext, PhotoContext>(
                context =>
                    {
                        if (!context.TryOpen())
                        {
